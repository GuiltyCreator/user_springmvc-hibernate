...
public class ImageUtils {
    ...
    public static final String DEFAULT_IMAGE_PATH = "/images/avatar/";

    private static String DEFAULT_SMALL_IMAGE_PATH = "/images/avatar/small/";

    /**
     * @Description 缩放图片
     * @param  source 源图片,targetW 缩放后的宽度,targetH 缩放后的长度;
    ...
    /**
     * @Description 等比缩放图片
     * @param  file 源图片,targetW 缩放后的宽度,targetH 缩放后的长度;
     * @return BufferedImage 缩放后的图片
     */
    ...
    public static Map<String, Object> saveImage(String rootPath, MultipartFile imageFile, boolean needSmallImage) throws Exception {
        ...
        if (null != imageFile && imageFile.getOriginalFilename().toLowerCase().trim().length() != 0) {
            ...
            System.out.println("图片路径为:"+rootPath + imageFileName);
            // 获取项目在磁盘上面的物理路径
